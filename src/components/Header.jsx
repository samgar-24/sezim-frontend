...
// 1. Принимаем onOpenAuth как пропс
    ...
    <header className="py-5 md:py-8 px-4 border-b border-[#BBBCB9]/20 bg-[#F0EEE9]/80 backdrop-blur-md sticky top-0 z-50">
      <div className="max-w-[1400px] mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 md:gap-10">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-4 md:gap-8">
                {(user?.is_staff || user?.username === 'admin') && (
                  <Link to="/admin-panel" className="text-[9px] md:text-[10px] font-medium bg-[#2B2929] text-[#F0EEE9] px-3 py-1.5 rounded-full tracking-widest hover:opacity-80 transition-all uppercase">
                    Admin
                  </Link>
                )}
                ...
                <button onClick={logout} className="text-[#BBBCB9] hover:text-[#2B2929] transition-colors">
              ...
              <button 
                onClick={onOpenAuth} // 2. Вызываем функцию из App.jsx
          ...
          <button onClick={toggleCart} className="relative flex items-center justify-center w-10 h-10 md:w-12 md:h-12 bg-[#2B2929] text-[#F0EEE9] rounded-full hover:scale-105 transition-all">
      ...
      {/* 3. Здесь БОЛЬШЕ НЕТ вызова AuthModal — он переехал в App.jsx */}
    </header>
  );
}